...
import { getHairstyleRecommendations } from '../src/data/hairstyleRecommendations';
...
export default function ResultScreen() {
  ...
  const faceShape = params.faceShape;
  ...
  // Compartilhar resultados (versão simplificada)
  const shareResults = async () => {
    Alert.alert("Funcionalidade de compartilhamento", 
  ...
  const saveToGallery = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert("Permissão Necessária", "Precisamos de permissão para salvar na sua galeria");
        return;
      }
      ...
      Alert.alert("Sucesso", "Imagem salva com sucesso na sua galeria!");
    } catch (error) {
      console.error("Erro ao salvar na galeria:", error);
      Alert.alert("Erro", "Não foi possível salvar sua imagem");
    }
  };
